...
@objc public final class ImageLoading {
    ...
    private func startTask(task: DiscardableTask<UIImage, NSError>, withRequest request: NSURLRequest) {
        ...
        let op = session.dataTaskWithRequest(request) {
            ...
            if let error = error {
                ...
                state = .Failure(error: Box(error))
            } else if let data = data, response = response, image = UIImage(data: data) {
                state = .Success(result: Box(image))
            }
